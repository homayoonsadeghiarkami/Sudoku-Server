...
class ClientHandler(Thread):
    def __init__(self, soc, Server):
        LOG.info('Created client {}:{}'.format(soc.getsockname()[0], soc.getsockname()[1]))
    ...
    def requestPutNumber(self, unparsedInts):
        ...
        LOG.debug('Client {}:{} wants to write to sudoku: {}'.format(
            self.soc.getsockname()[0], self.soc.getsockname()[1], unparsedInts)
        )
        try:
    ...
    def rcvMessage(self):
        ...
        try:
            ...
            if len(b) <= 0:
                LOG.info('Client %s:%d Disconnected'.format(
                    self.soc.getsockname()[0], self.soc.getsockname()[1])
                )
                self.soc.close()
                m = ''
            m = m[:-1]
            ...
            self.soc.close()
            LOG.info('Ctrl+C issued, disconnecting client %s:%d'.format(
                    self.soc.getsockname()[0], self.soc.getsockname()[1])
                )
            m = ''
        except soc_err as e:
            if e.errno == 107:
                LOG.warning(
                    'Client %s:%d left before server could handle it'.format(
                     self.soc.getsockname()[0], self.soc.getsockname()[1]
                    )
                )
            else:
    ...
    def createSession(self, sessName, maxPlayerCount):
        ...
        if maxPlayerCount < 2:
            return REP_NOT_OK, f"Too few max players specified {maxPlayerCount}"
    ...
    def rcvProtocolMessage(self, message):
        ...
        if message.startswith(REQ_NICKNAME + HEADER_SEP):
            if payload not in self.Server.getUsedNicknames():
                ...
                LOG.debug('Client {}:{} will use name {}'.format(
                 self.soc.getsockname()[0], self.soc.getsockname()[1], self.nickname)
                )
    ...
    def session_send(self, msg):
        ...
        with self.send_lock:
            r = False
            try:
                self.soc.sendall(m.encode('utf-8'))
                r = True
            except KeyboardInterrupt:
                self.soc.close()
                ...
                if e.errno == 107:
                    LOG.warning('Client %s left before server could handle it' \
                             '' % self.soc.nickname)
                else:
                    LOG.error('Error: %s' % str(e))
                self.soc.close()
